...
const Election = () => {
  // State to hold all polls fetched from the backend
  const [polls, setPolls] = useState([]);

  // State to track which polls the user has voted in
  const [votes, setVotes] = useState({});

  // Loading state for data fetching
  const [loading, setLoading] = useState(true);

  // Stores selected option index for each poll (used to highlight voted option)
  ...
  // Function to fetch all polls from backend
  ...
  const handleVote = async (pollId, optionIndex) => {
    ...
    try {
      // Verify if poll exists and is not closed
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <div className="grid gap-8">
              {polls.length > 0 ? (
                polls.map((poll) => {
                  const chartData = prepareChartData(poll); // compute chart data safely here
                  ...
                  return (
                    ...
                    >
                      <div className="p-8">
                        ...
                        {/* Results Section - only shown if user has voted */}
                        {votes[poll._id] ? (
